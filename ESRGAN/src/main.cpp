...
cv::Mat convertTfLiteTensorToCvMat(const TfLiteTensor* tensor) {
    ...
    if (tensor->dims->size != 4 || tensor->dims->data[0] != 1 || 
        tensor->dims->data[1] != 200 || tensor->dims->data[2] != 200 || tensor->dims->data[3] != 3) {
    ...
    if (tensor->type == kTfLiteFloat32) {
        ...
        cv::Mat float_mat(200, 200, CV_32FC3, data);
        ...
        mat = cv::Mat(200, 200, CV_8UC3, data).clone(); // Clone to create a separate copy
    } else {
...
int main(int argc, char** argv) {    
    ...
    const char *model_path = "../models/ESRGAN.tflite";    
    const char *input_image_path = "../lr-1.jpg";
