...
cv::Mat convertTfLiteTensorToCvMat(const TfLiteTensor* tensor) {
    // Ensure the tensor shape is [1, 1088, 1920, 3]
    if (tensor->dims->size != 4 || tensor->dims->data[0] != 1 || 
        tensor->dims->data[1] != 1088 || tensor->dims->data[2] != 1920 || tensor->dims->data[3] != 3) {
        std::cerr << "Unexpected tensor shape: ";
        for (int i = 0; i < tensor->dims->size; ++i) {
            std::cout << tensor->dims->data[i] << " ";
        }
        std::cerr << std::endl;
        throw std::runtime_error("Tensor shape is not [1, 1088, 1920, 3]");
    }
    ...
    if (tensor->type == kTfLiteFloat32) {
        ...
        cv::Mat float_mat(1088, 1920, CV_32FC3, data);
        ...
        mat = cv::Mat(1088, 1920, CV_8UC3, data).clone(); // Clone to create a separate copy
    } else {
...
int main(int argc, char** argv) {    
    ...
    const char *model_path = "../models/reseffnet_gan_div4_float16.tflite";    
    const char *input_image_path = "../input.jpg";
    const char *output_image_path = "../output_div4_f16.jpg";
    ...
    /*
    // Conver TfLite to Torch 
    torch::Tensor torch_tensor = tf_lite_to_torch_tensor(output_tensor);

    //Remove batch dimension
    torch_tensor = torch_tensor.squeeze(0); // [1080, 1920, 3]
